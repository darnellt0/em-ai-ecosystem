...
export class HybridRouterService {
  ...
  private simpleIntents = {
    'block-focus': {
      keywords: ['block', 'focus', 'deep work', 'heads down', 'no interruptions'],
      confidence: 0.95
    },
    'schedule-meeting': {
      keywords: ['schedule', 'meeting', 'call', 'appointment', 'calendar', 'book'],
      confidence: 0.95
    },
    'complete-task': {
    ...
    'daily-brief': {
      keywords: ['brief', 'summary', 'today', 'how am i doing', 'productivity', 'stats'],
      confidence: 0.90
    },
    'pause-meditation': {
  ...
  private constructor() {
    ...
    if (!apiKey) {
      console.warn('⚠️ OPENAI_API_KEY not configured. OpenAI fallback disabled.');
  ...
  async route(request: HybridRequest): Promise<HybridResponse> {
    ...
    try {
      ...
      if (complexity === 'simple' || (complexity === 'moderate' && intent)) {
        try {
          const result = await this.tryDeterministicAgent(intent || '', request);
          if (result) {
      ...
      if (!this.openai) {
        return {
          route: 'deterministic',
          complexity,
          humanSummary: '❌ Request too complex and OpenAI not configured. Try a simpler command like "block focus for 30 minutes"',
      ...
      return {
        route: 'deterministic',
        complexity: 'complex',
        humanSummary: '❌ An error occurred processing your request. Please try again.',
   ...
   */
  private detectComplexity(transcript: string): { complexity: 'simple' | 'moderate' | 'complex'; intent?: string } {
  ...
  private async tryDeterministicAgent(
    ...
    const intentMap: Record<string, () => Promise<any>> = {
      ...
      'complete-task': () => agentFactory.logTaskComplete(
        founderEmail,
        'task-1',
        request.transcript
      ),
      ...
      'pause-meditation': () => agentFactory.startPause(
        founderEmail,
        'grounding',
        this.extractNumber(request.transcript, 60)
      ),

      'follow-up': () => agentFactory.createFollowUp(
        founderEmail,
        this.extractTitle(request.transcript),
        undefined,
        request.transcript
