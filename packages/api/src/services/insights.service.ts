...
export class InsightsService {
  ...
  async generateDailyBrief(founderEmail: string): Promise<{
    ...
    try {
      ...
      sections.push({
        ...
        actionItems:
          metrics.energyLevel === "low"
            ? ["Take a break", "Go for a walk", "Hydrate"]
            : [],
      });
      ...
      sections.push({
        ...
        priority: "medium",
        actionItems: [
          this.getProductivityRecommendation(metrics.productivityScore),
        ],
      });
  ...
  async getInsights(
    ...
    try {
      ...
      this.logger.info(
        `[Insights] Generating ${timeframe} insights for ${founderEmail}`,
      );

      const stats = await databaseService.getActivityStats(
        founderEmail,
        startDate,
        now,
      );
      ...
      // Focus insight
      if (
        stats.byType["deep-work"] !== undefined ||
        stats.byType["focus"] !== undefined
      ) {
        const focusMinutes =
          stats.byType["deep-work"] || stats.byType["focus"] || 0;
        insights.push({
          type: "focus",
          ...
          recommendation:
            focusMinutes > 120
              ? "Excellent focus sessions! Keep up the momentum."
              : "Try to increase daily focus time.",
        });
      }

      // Pause insight
      if (
        stats.byType["pause"] !== undefined ||
        stats.byType["meditation"] !== undefined
      ) {
        const pauseMinutes =
          stats.byType["pause"] || stats.byType["meditation"] || 0;
        insights.push({
          type: "pause",
          ...
          recommendation:
            pauseMinutes > 20
              ? "Great self-care routine!"
              : "Consider adding more breaks to prevent burnout.",
        });
      }
  ...
  private async getDailyMetrics(
    ...
    const focusMinutes =
      stats.byType["focus"] || stats.byType["deep-work"] || 0;
    const pauseMinutes =
      stats.byType["pause"] || stats.byType["meditation"] || 0;
    const tasksCompleted = Math.max(
      1,
      Math.floor((stats.byType["task-completion"] || 0) / 30),
    );
    const meetingsAttended = Math.max(
      0,
      Math.floor((stats.byType["meeting"] || 0) / 30),
    );
   ...
   */
  private generateEnergyRecommendation(
    level: string,
    pauseMinutes: number,
  ): string {
    if (level === "low") {
  ...
  async detectPatterns(
    ...
    try {
      this.logger.info(
        `[Insights] Detecting patterns for ${founderEmail} over ${days} days`,
      );
  ...
  async getRecommendations(founderEmail: string): Promise<string[]> {
    try {
      this.logger.info(
        `[Insights] Generating recommendations for ${founderEmail}`,
      );
