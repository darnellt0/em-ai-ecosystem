...
interface Props {
  feature: LeadershipFeature | null;
  onClose: () => void;
}
...
export const InteractionView = ({ feature, onClose }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/em-ai/leadership-session', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error('Failed to reach Leadership Session endpoint');
      }
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-300/80">Leadership Hub</p>
            ...
            <div className="space-y-3 mb-16 max-h-[50vh] overflow-y-auto pr-2">
              {messages.length === 0 && (
                <div className="text-sm text-slate-300">
                  Welcome to {feature.title}. What’s present in your leadership and rest rhythm today?
                </div>
              )}
            ...
            <form onSubmit={handleSubmit} className={`fixed left-0 right-0 mx-6 bottom-8 ${feature.shadow}`}>
              <div className="max-w-4xl mx-auto flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 backdrop-blur px-4 py-3">
                <input
                  ...
                  placeholder="I’m leading a lot and want to stay rested and intentional..."
